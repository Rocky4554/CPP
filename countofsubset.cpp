...
int check(int ar[],int n,int sum){//change bollto int to get return in inetrger no.
    ...
    for(int i=1;i<n+1;i++){
        for(int j=1;j<sum+1;j++){
            if(ar[i-1]>sum){
                ...
                s[i][j]=s[i-1][j-ar[i-1]] + s[i-1][j]; //only changind "||"to "+"" to get total subset.
        }
    }
